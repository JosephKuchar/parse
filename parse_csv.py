...
def parse_csv(df,addr_col):
    df[['LP_Number', 'LP_Street', 'LP_City', 'LP_Province', 'LP_PostCode']]=df.apply(lambda x: format_parser(x[addr_col]), axis=1, result_type='expand')
    return df
...
def format_parser(add):
    ...
    key_list = ['house_number', 'road', 'city', 'state', 'postcode']
    parsed = []
    for k in key_list:
        if k in B.keys():
            parsed.append(B[k])
